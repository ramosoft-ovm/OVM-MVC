$(function(){
    ...
    function updateExpirationDate(dataSet) {
        ...
        {            
            if (localStorage.getItem("isCBCheckedLocal")){
                app.showNotificactionVBC('Su sesión ha expirado');
                localStorage.clear();    
            }
        }
    }
    ...
    $("#saveSettings").click(function(){
        ...
        if (document.getElementById('regId').value != "") {
            ...
            if (document.getElementById('username').value != "" && document.getElementById('password').value != "") {
                ...
                function validateUser(dataSet){
                    ...
                    if(rec['status'] == 0){
                                ...
                                if (isNaN(username)) {
                                    ...
                                    function getUserId(dataSet) {
                                        ...
                                        function updateStatus(dataSet) {
                                            ...
                                            if (status === 0) {
                                                app.showNotificactionVBC('Tu cuenta ha sido bloqueada por exceder el límite de intentos fallidos, comunícate con el Administrador para desbloquearla.');
                        ...
                        app.showNotificactionVBC('El password es Incorrecto ('+localStorage.getItem('intentos'+username)+')');
            ...
            //app.showNotificactionVBC('Esperando regId');
        }
        ...
        if(!userId == ''){
            ...
            function getPassword(dataSet){
                ...
                if(typeof(rec) == 'undefined'){
                    $('#txtUserId').val("");
                    app.showNotificactionVBC('El Número de Usuario que usted digitó no existe');
                    //app.showNotificactionVBC('userid no existe');
                }else if(!rec['email'] == ''){                    
                    var parametros = {
                        "EMAIL" : rec['email'],
                    ...
                    $.ajax({
                        ...
                        success: function(response){
                            ...
                            if(response == "1"){                        
                                ...
                                app.showNotificactionVBC('El envío de la contraseña al correo '+ rec['email'] +' ha sido exitoso');
                            }else{
                                app.showNotificactionVBC('El envío de la contraseña ha fallado, intente nuevamente');
                            }
                        },
                        error: function(response){
                            console.log('Error '+ response);
                            app.showNotificactionVBC('Ha ocurrido un error en el envío de la contraseña');
                        }
                    });
                }
            }
        }else{
            app.showNotificactionVBC('El campo del Número de Usuario no puede estar vacío');
        }
